...
| F3 | `d5a9bc105` | feat: `annual_taxable_amount_after_exemption` on Salary Slip — the annual taxable amount less income exempted by zero-rate Income Tax Slab rows, so a formula on the taxed base works whether the exemption is a standard exemption or a 0 % slab row | pending (forward-port to `develop`) |
